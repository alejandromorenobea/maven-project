pipeline {
    ...
    environment {
        repoApp= "https://github.com/alejandromorenobea/maven-project.git"
        rama = "master"

        dateTime = sh(returnStdout: true, script: 'date +%Y%m%d-%H%M').trim()
        imagen = "alejandromorenobea1/testing-jenkins:${dateTime}"
    }
                ...
                dir(path: 'contenedor') {
                    withCredentials(bindings: [usernamePassword(credentialsId: 'github_credential', 
                        ...
                        sh '''
                            ...
                            git add .
                            git config --global user.email \'alejandromorenobea@fp.iesromerovargas.com\'
                            git config --global user.name  \'alejandromorenobea\'
